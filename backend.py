...
def isentropic_calculate(gamma, input_type, input_value, is_sub=None):
    """
    Performs isentropic flow calculations.
    Can solve for all properties from any single input property.
    """
    ...
    # --- Step 1: Find Mach number (M) from the given input ---
    m = 0
    if input_type == "mach":
        m = input_value
        ...
        m = bisection_solve(pm_func, input_value, 1.0001, 80)
        ...
        if is_sub:
            ...
            m = bisection_solve(area_func, input_value, 1.0001, 80)

    # --- Step 2: Compute all output properties from M ---
...
def normal_shock_calculate(gamma, input_type, input_value):
    """
    Performs normal shock calculations.
    Can solve for all properties from M1, M2, or any property ratio.
    """
    # --- Step 1: Find upstream Mach number (M1) from the given input ---
    def m1_from_m2(m2, g):
    ...
    def bisection_solve(func, target, low, high, tol=1e-8):
        # General purpose solver for monotonic functions
        is_increasing = func(high) > func(low)
        while high - low > tol:
            mid = (low + high) / 2
            val = func(mid)
            if (is_increasing and val < target) or (not is_increasing and val > target):
                low = mid
            else:
                high = mid
        return (low + high) / 2
    ...
    if input_type == "m1":
        ...
        def t_ratio_func(m_val):
            p_r = (2 * g * m_val**2 - (g - 1)) / (g + 1)
            rho_r = ((g + 1) * m_val**2) / ((g - 1) * m_val**2 + 2)
            return p_r / rho_r
        m1 = bisection_solve(t_ratio_func, input_value, 1.0001, 80)
    elif input_type == "po2_po1":
        if input_value >= 1: raise ValueError("Po2/Po1 must be < 1.")
        def po_ratio_func(m_val):
            t1 = ((g + 1) * m_val**2) / ((g - 1) * m_val**2 + 2)
            t2 = (g + 1) / (2 * g * m_val**2 - (g - 1))
            return t1**(g / (g - 1)) * t2**(1 / (g - 1))
        m1 = bisection_solve(po_ratio_func, input_value, 1.0001, 80)
    else:
        raise ValueError(f"Invalid input type for Normal Shock: {input_type}")

    # --- Step 2: Compute all properties from M1 ---
    ...
    return {
        ...
        "T2/T1": t2_t1, "Po2/Po1": po2_po1,
    }


def oblique_shock_calculate(gamma, m1, input_type, input_value, is_weak=None):
    """
    Performs oblique shock calculations.
    Can solve from M1 and Turn Angle, Wave Angle, or M1n.
    """
    if m1 <= 1: raise ValueError("M1 must be supersonic (> 1).")
    ...
    # --- Step 1: Find wave angle (beta) and turn angle (theta) ---
    beta, theta = 0, 0
    if input_type == "theta":
        ...
        # M1n = M1 * sin(beta) -> beta = asin(M1n / M1)
    ...
    # --- Step 2: Compute all properties from M1, beta, and theta ---
